TEST_CASE("UCB-based adaptation favors successful operators", "[calibration][ucb]") {
  ...
  auto goodOperator = []([[maybe_unused]] auto* eva, const std::vector<std::shared_ptr<const Permutation>>& parents) -> Values {
    ...
    // Swap adjacent pairs if out of order - makes it better
  ...
  auto badOperator = []([[maybe_unused]] auto* eva, const std::vector<std::shared_ptr<const Permutation>>& parents) -> Values {
    Values values = parents[0]->values;
    // Swap adjacent pairs if in order - makes it worse
    for (size_t i = 0; i < values.size()-1; i++) {
      if (values[i] < values[i+1]) {
        std::swap(values[i], values[i+1]);
      }
  ...
  EVA::EvolutionaryAlgorithm<Permutation, Values> eva({
    ...
    .threadConfig = {
      .spawn = {spawnReverse, EVA::constructor<Permutation, Values>()},
      .reproduction = {
        { {EVA::randomSelection<Permutation, Values>()}, goodOperator, EVA::constructor<Permutation, Values>() },
        { {EVA::randomSelection<Permutation, Values>()}, badOperator, EVA::constructor<Permutation, Values>() }
      },
      .calibration = [&weights](auto* eva, const auto& offspring, size_t reproducer, const auto& fitness, bool isUnfit, bool isDuplicate, bool isFittest) {
        EVA::ucbBasedAdaptation<Permutation, Values>()(eva, offspring, reproducer, fitness, isUnfit, isDuplicate, isFittest);
